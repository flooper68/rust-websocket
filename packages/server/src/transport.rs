...
use crate::session::{SessionEvent, SessionCommand, MoveRectangleCommand, Rectangle, RectangleCreatedCommand};
...
pub fn translate_event_to_ws(event: SessionEvent) -> String {
    match event {
        ...
        SessionEvent::RectangleCreated(event) => {
            let serialized = serde_json::to_string(&event).unwrap();
            encode_message("RectangleCreated", &serialized)
        }
    }
}
...
pub fn translate_ws_to_command(message: String) -> SessionCommand {
    ...
    match kind {
        ...
        "CreateRectangle" => {
            println!("Parsing CreateRectangle {}", message);
            let parsed_message: RectangleCreatedCommand = serde_json::from_str(parts[1]).unwrap();
            let command = SessionCommand::CreateRectangle(parsed_message);
            command
        }
