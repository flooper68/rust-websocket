...
pub struct RectangleCreatedCommand {
    uuid: String,
    left: f32,
    top: f32,
    width: u32,
...
impl RectangleCreatedCommand {
    pub fn new(uuid: String, left: f32, top: f32, width: u32, height: u32) -> RectangleCreatedCommand {
        RectangleCreatedCommand {
          uuid,
          left,
          top,
          width,
...
pub enum SessionCommand {
    ...
    CreateRectangle(RectangleCreatedCommand)
}
...
#[derive(Serialize, Deserialize)]
#[derive(Clone)]
pub struct RectanglCreatedEvent {
    pub uuid: String,
    pub left: f32,
    pub top: f32,
    pub width: u32,
...
impl RectanglCreatedEvent {
    pub fn new(uuid: String,  left: f32, top: f32, width: u32, height: u32) -> RectanglCreatedEvent {
        RectanglCreatedEvent {
            uuid,
            left,
            top,
            width,
...
pub enum SessionEvent {
    ...
    RectangleCreated(RectanglCreatedEvent)
}
...
impl SessionState {
    ...
    fn create_rectangle(&mut self, uuid: String, left: f32, top: f32, width: u32, height: u32) -> SessionEvent {
        let rectangle = Rectangle::new(uuid, width, height, left, top);
        println!("Rectangle {} created", rectangle.uuid.clone());
        let event =SessionEvent::RectangleCreated(RectanglCreatedEvent::new(rectangle.uuid.clone(), left, top, width, height));
        self.rectangles.insert(rectangle.uuid.clone(), rectangle);
        event
    }
}
...
pub async fn start_session(mut session_command_receiver: UnboundedReceiver<SessionCommand>) {
    ...
    while let Some(command) = session_command_receiver.recv().await {
        match command {
            ...
            SessionCommand::CreateRectangle(command) => {
                let event = session_state.create_rectangle(command.uuid,command.left, command.top, command.width, command.height);
                dispatch_event(event, &session_state.connections);
            }
        }
    }
